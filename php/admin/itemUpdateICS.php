...
$sql = "SELECT acctSn,pgsoSn,description,units.unitName,unitCost,brand,supplierID,categoryNo,icsNo FROM items JOIN units ON items.unitID = units.unitID WHERE itemID = '$id'";
$res = $conn->query($sql);
$r = $res->fetch_row();
...
if($_POST['icsNo'] != $ics) {
    $ics = $_POST['icsNo'];
    $act = "Updated ICS from " . $r[8] . " to " . $ics;
    $sql = "INSERT INTO updatehistory(accountID,activity,time,date,type,itemID) VALUES ('$userID','$act','$t','$d','Item Update','$id')";
    $conn->query($sql);
}

$sql = "UPDATE items SET icsNo='$ics',categoryNo='$category',acctSn = '$acct',pgsoSn = '$pgso',description = '$des',unitID = '$unit',unitCost = '$cost',brand = '$brand',supplierID = '$supplier' WHERE itemID = '$id'";
